...
app.post('/api/sessions/import', async (req, res) => {
  try {
    ...
    for (const session of sessions) {
      await saveSession(session.profileId, session.userAgent, session.cookies);
    }
    res.json({ success: true, message: `Imported ${sessions.length} sessions` });
  } catch (error) {
    res.json({ success: false, message: error.message });
  }
});
...
app.get('/download-sample', (req, res) => {
  res.download(path.join(__dirname, 'sample_profiles.csv'), 'sample_profiles.csv');
});

// Start the server if not being imported by another module (e.g. electron.js)
...
// Export app for use in electron.js
module.exports = app;